...
export const Game2: React.FC<Game2Props> = ({ customItems, onGameComplete, onLevelChange, searchQuery }) => {
          ...
          return (
            ...
            >
              <div className={`relative w-full h-full duration-500 transition-transform preserve-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
                ...
                <div className={`absolute inset-0 backface-hidden rotate-y-180 ${card.color || 'bg-indigo-50'} border-2 border-indigo-400 rounded-xl flex flex-col items-center justify-center shadow-[0_4px_0_rgba(79,70,229,0.5)] overflow-hidden transition-all p-1`}>
                  {card.imageUrl ? (
                    <img
                      src={card.imageUrl}
                      alt={card.name}
